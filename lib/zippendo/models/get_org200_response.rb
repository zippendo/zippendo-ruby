...
module Zippendo
  class GetOrg200Response < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zippendo::GetOrg200Response` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zippendo::GetOrg200Response`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] _count Value to be assigned
    def _count=(_count)
      if _count.nil?
        fail ArgumentError, '_count cannot be nil'
      end
    ...
    def hash
      [id, name, slug, description, currency, vat_number, billing_email, company_name, address_line1, address_line2, city, postal_code, country, customs, created_at, updated_at, _count].hash
    end
