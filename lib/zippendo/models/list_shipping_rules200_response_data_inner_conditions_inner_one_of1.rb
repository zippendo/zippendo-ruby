...
module Zippendo
  class ListShippingRules200ResponseDataInnerConditionsInnerOneOf1 < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Zippendo::ListShippingRules200ResponseDataInnerConditionsInnerOneOf1` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Zippendo::ListShippingRules200ResponseDataInnerConditionsInnerOneOf1`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["price"])
      return false unless type_validator.valid?(@type)
      return false if @price_type.nil?
      price_type_validator = EnumAttributeValidator.new('String', ["subtotal", "total"])
    ...
    def price_type=(price_type)
      validator = EnumAttributeValidator.new('String', ["subtotal", "total"])
    ...
    def hash
      [type, price_type, operator, value, shipping_price, currency].hash
    end
